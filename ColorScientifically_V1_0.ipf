...
Function ColorScientificallyPanel()
	setdatafolder root: //so you don't have multiples of the same waves
	...
	//creates variables
	if(exists("ColorMapStr") == 0)
		String/g ColorMapStr
	else
		SVAR ColorMapStr
	endif
	...
	//Color Preview Plot
	Display/Host=ColorScientifically/N=ColorPreview/W=(-10, 50, 400, 230); Modifypanel cbrgb= (0,0,0); Make/O/N=(25,1) PlotColorScheme = x; AppendImage PlotColorScheme; 
	Make/O/N=(25,3) RedGreenBlue=NaN; setdimlabel 1,0, Red, RedGreenBlue; setdimlabel 1,1 , Green, RedGreenBlue; setdimlabel 1,2 , Blue, RedGreenBlue
   //show first color
	...
	button DoIt, size={120,40}, pos={100,260},proc=ColorScientificallyProc, title="Color Scientifically!", win=ColorScientifically //The Color Scientifically button
End
...
Function/S ColorList()
	...
	listofcolors+= "berlin (blue/red);"
	listofcolors+= "bilbao (white/yellow/red);"
	listofcolors+= "broc (snot to white to blue);"
	listofcolors+= "buda (yellow to pink);"
	listofcolors+= "cork (green to white to blue);"
	listofcolors+= "davos (white to green to blue);"
	listofcolors+= "devon (white to purple to blue);"
	listofcolors+= "grayC (white to grey to black);"
	listofcolors+= "hawaii (blue to yellow to pink);"
	listofcolors+= "imola (yellow to green to blue);"
	listofcolors+= "lajolla (black to red to yellow);"
	listofcolors+= "lapaz (peach to green to blue);"
	listofcolors+= "lisbon (yellow to black to blue);"
	listofcolors+= "nuuk (yellow to blue);"
	listofcolors+= "oleron (green and blue);"
	listofcolors+= "oslo (blue);"
	listofcolors+= "roma (blue to yellow to red);"
	listofcolors+= "romaO (purple to yellow to red);"
	listofcolors+= "tofino (green to purple);"
	listofcolors+= "tokyo (snot to purple);"
	listofcolors+= "turku (pink to snot);"
	listofcolors+= "vik (red to white to blue);"
	listofcolors+= "vikO (red to blue);"
...
Function ColorSchemeSet(PU_Struct) : PopupMenuControl
	...
	switch(PU_Struct.eventcode)
		case 2: //mouse up
			string/G colormapstr = PU_Struct.popstr
			variable/G colormap = PU_Struct.popnum-1
	...
	switch(event)    
		case 2:     // handle left mouse click         
			...
			elseif(colormap==16) //nuuk //yellow to blue
				RedGreenBlue[][0]= {5,23,34,45,57,69,83,96,111,125,138,150,161,169,176,181,186,190,195,201,210,220,232,243,254}
...
Function ColorScientificallyProc(ba) : ButtonControl
	...
	Make/O/N=50/T colorselect = {"acton", "bamako", "batlow", "berlin", "bilbao", "broc", "buda", "cork", "davos", "devon", "grayC"}
	NVAR Colormap
	...
	EndIf
	RedGreenBlue *=257
	string listtrcs = TraceNameList("",";",1+4); variable numtrcs = ItemsinList(listtrcs,";") //only visible trace colors are changed!!
	
	//Evenly space the colors and change colors
	variable separat = floor(dimsize(Redgreenblue,0)/numtrcs); variable itrc
	For(itrc=0; itrc<numtrcs; itrc++)
		string currtrc = stringfromlist(itrc, listtrcs); variable trcnum = separat*itrc
		Modifygraph rgb($currtrc)=(RedGreenBlue[trcnum][0], RedGreenBlue[trcnum][1], RedGreenBlue[trcnum][2]) //change the color
	EndFor
End	