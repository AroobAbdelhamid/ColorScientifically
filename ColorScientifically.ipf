...
Function ColorScientificallyPanel()
	...
	if(!datafolderexists("root:ColorScientifically"))
		NewDataFolder root:ColorScientifically
	endIf
	
	//creates variables if necessary
	SVAR/Z ColorMapStr = root:ColorScientifically:ColorMapStr
	if(!SVAR_exists(ColorMapStr))
		String/g root:ColorScientifically:ColorMapStr
	endif
	
	NVAR/Z ColorMap = root:ColorScientifically:ColorMap; ColorMap=0
	//creates variables
	if(!NVAR_exists(ColorMap))
		Variable/g root:ColorScientifically:ColorMap
	endif
	
	NVAR/Z RevCol = root:ColorScientifically:RevCol; RevCol=0
	//creates variables
	if(!NVAR_exists(RevCol))
		Variable/g root:ColorScientifically:RevCol
	endif

	SVAR/Z WinNm = root:ColorScientifically:WinNm; WinNm = ""
	if(!SVAR_exists(WinNm))
		String/g root:ColorScientifically:WinNm; 
	endif
	...
	SetVariable/Z WindowName size={300, 20}, Pos={25,40}, proc=WinNmset, title="Window Name (optional)", win=ColorScientifically, value= root:ColorScientifically:WinNm

	//checkbox
	CheckBox/Z ReverseCol size={100, 20}, Pos={25,65}, proc=ReverseColors, title="Reverse Colors?", win=ColorScientifically

	//Color Preview Plot
	Display/Host=ColorScientifically/N=ColorPreview/W=(-30, 85, 400, 280); Modifypanel cbrgb= (0,0,0); 
	Make/O/N=(25,1) PlotColorScheme = x; AppendImage PlotColorScheme; 
	Make/O/N=(25,3) root:ColorScientifically:RedGreenBlue/WAVE=RedGreenBlue; RedGreenBlue = NaN
...
Function ColorGraph(WinNm)
	...
	wave RedGreenBlue = root:ColorScientifically:RedGreenBlue
	...
	NVAR/Z RevCol = root:ColorScientifically:RevCol
	If(RevCol ==0) //if reverse colors not checked
		variable sttrc=0, endtrc=numtrcs
	else //if reverse colors checked
		sttrc=-numtrcs+1; endtrc=1
	EndIf
...
Function ColorSchemeSet(PU_Struct) : PopupMenuControl
	...
	switch(PU_Struct.eventcode)
		case 2: //mouse up
			string/G root:ColorScientifically:colormapstr = PU_Struct.popstr
			variable/G root:ColorScientifically:colormap = PU_Struct.popnum-1
...
//Reverse colors?
Function ReverseColors(cba) : CheckBoxControl
	STRUCT WMCheckboxAction &cba

	switch( cba.eventCode )
		case 2: // mouse up
			NVAR RevCol = root:ColorScientifically:RevCol
			RevCol = cba.checked
			break
		case -1: // control being killed
			break
...
Function UpdateTheDisplay(event)
	...
	switch(event)    
		case 2:     // handle left mouse click         
			NVAR colormap = root:ColorScientifically:colormap
...
Function ColorScientificallyProc(ba) : ButtonControl
	...
	switch( ba.eventCode )
		case 2: // mouse up
			NVAR/Z Colormap = root:ColorScientifically:colormap
			RedGreenBlueFn(colormap) //what color scheme
			SVAR/Z WinNm = root:ColorScientifically:WinNm
...
Function/S ColorList()
	...
	listofcolors+= "davos (blueyellow/white);"
...
Function RedGreenBlueFn(colormap)
	...
	case 3: //berlin //blue to red
		...
		break
	case 4: //bilbao //white to yellow to red
		...
		break
	case 5: //broc: snot to white to blue
	...
	case 6: //buda // yellow to pink
		...
		break
	case 7: //cork //green to white to blue
		...
		break
	case 8: //davos //white to green to blue
		...
		break
	case 9: //devon //white to purple to blue
		...
		break
	case 10: //greyC //white to grey to black
		...
		break
	case 11: //hawaii //blue to yellow to pink
		...
		break
	case 12: //imola //yellow to green to blue
		...
		break
	case 13: //lajolla //black to red to yellow
		...
		break
	case 14: //lapaz //peach to green to blue
		...
		break
	case 15: //lisbon //white to black to blue
	...
	case 18: //oslo //blue
		...
		break
	case 19: //roma //blue to yellow to red
		...
		break
	case 20: //romaO //purple to yellow to red
		...
		break
	case 21: //tofino //green to purple
	...
	case 23: //turku //pink to snot
		...
		break
	case 24: //vik //red to white to blue
